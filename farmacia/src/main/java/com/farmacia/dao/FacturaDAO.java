...
public class FacturaDAO implements IFacturaProveedorDAO {
	...
	@Override
	public int eliminarFacturaProveedor(int cod_FacturaProveedor) {
		//Declarar una variable para el resultado
		int r = -1;
		...
		try {
			...
			cstm.setInt(1, cod_FacturaProveedor);
	...
	public FacturaProveedor buscarFacturaProveedor(int cod_factura) {
		...
		try {
			...
			while(rs.next()) {
				...
				f.setCod_factura(cstm.getInt(1));
				f.setCod_prov(cstm.getInt(2));
				f.setCod_emp(cstm.getInt(3));
				f.setFecha_em(cstm.getString(4));
				f.setMetodo_pago(cstm.getString(5));
			}
	...
	public ArrayList<FacturaProveedor> listadoFacturaProveedor() {
		...
		try {
			...
			while(rs.next()) {
				...
				f.setCod_prov(cstm.getInt(2));
				f.setCod_emp(cstm.getInt(3));
				f.setMetodo_pago(cstm.getString(4));
				f.setFecha_em(cstm.getString(5));
				data.add(f);
			}
