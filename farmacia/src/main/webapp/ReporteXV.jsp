...
<%@page import="com.farmacia.entidad.Empleado"%>
...
<body>
	<c:set var="lista_emp" value="<%= new EmpleadoDAO().listadoEmpleado() %>" />
	<div class="container">
		<%@ include file="componentes/menu.jsp" %>
		<div class="row mt-4">
			<div class="col-md-9">
				<h1>Reporte Ventas por Vendedor</h1>				
			</div>
		</div>
		<div class="mt-4 row">
    	<div class="col form-group ns">
    		<label for="cboEmpleado" class="form-label">Vendedor</label>
	      <select class="selectpicker w-100" name="empleado" data-placeholder="Elije un vendedor" data-live-search="true" id="cboEmpleado">
				  <c:forEach var="emp" items="${pageScope.lista_emp}">
				  	<option value="${emp.getCod_emp()}"> ${emp.getCod_emp()} -- ${emp.getNom_emp()} ${emp.getApe_emp()} -- DNI: ${emp.getDni_emp()}</option>
				  </c:forEach>
				</select>
    	</div>
    </div>
  ...
  <div class="modal-dialog modal-xl modal-dialog-centered">
	...
	<script type="text/javascript">
		$(document).ready(function(){
			...
			function mostrarVentasEmpleado(codEmp){
